...
const teams = {
    ...
    'ENG': { name: 'Angleterre', flag: '🏴' },
    'IRN': { name: 'Iran', flag: '🇮🇷' },
    'USA': { name: 'USA', flag: '🇺🇸' },
    'WAL': { name: 'Pays de Galles', flag: '🏴' },
    ...
    'TAH': { name: 'Tahiti', flag: '🇵🇫' },
    'FIJ': { name: 'Fidji', flag: '🇫🇯' },
    'PNG': { name: 'Papouasie-Nouvelle-Guinée', flag: '🇵🇬' },
    'SCO': { name: 'Écosse', flag: '🏴' },
...
// Rendu d'un match (ligne du calendrier)
...
// Affichage de tous les matchs (calendrier)
function renderAllMatches(filter = 'all') {
    ...
    if (filtered.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:#888;">Aucun match pour ce filtre.</p>';
        return;
    }
...
// Affichage des meilleurs buteurs (classement)
    ...
    container.innerHTML = scorers.map((s, index) => `
        <div class="scorer-row">
            ...
            <div>
                ...
                <div class="scorer-country">${(teams[s.country] && teams[s.country].name) || s.country}</div>
            </div>
            <div></div>
...
// Gestion des boutons de filtre du calendrier
